...
def main():
    ...
    data_split_path = '/data/0shared/lijun/code/ECGFounder_CLIP/finetune/data_split'
    data_meta_path = '/data/0shared/lijun/code/ecg_ptbxl_benchmarking/data'
    ...
    if 'ptbxl' in args.dataset:
        ...
        data_path = f'{data_meta_path}/cpsc2018'
    ...
    if 'Net1D_18' in args.backbone:
        if args.backbone == 'Net1D_18':
            model = Net1D_18(num_classes=num_classes)
        
        model.load_state_dict(ckpt, strict=False)
        print(f'load pretrained model from {args.pretrain_path}, the backbone is {args.backbone}, using {args.num_leads} leads')
        if 'dense' in args.name:
            for param in model.parameters():
                param.requires_grad = False
            print(f'freeze backbone for {args.name} with {args.backbone}')
    ...
    if 'CLIP_ResNet' in args.backbone:
        ...
        print(f'Loaded pretrained model from {args.pretrain_path}, backbone is {args.backbone}, using {args.num_leads} leads')
...
def infer(model, loader, args):
    ...
    for step, (ecg, target) in tqdm(enumerate(loader)):
        ...
        input_label_list = target.to('cuda:2')

        predictions = model(ecg.to('cuda:2'))

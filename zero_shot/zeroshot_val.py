...
def get_class_emd(model, class_name, device='cuda:6'):
    model.eval()
    with torch.no_grad(): # to(device=torch.device("cuda"iftorch.cuda.is_available()else"cpu")) 
        ...
        for texts in class_name:
            ...
            texts = clip.tokenize(texts).cuda(6)
...
def get_ecg_emd(model, loader, zeroshot_weights, device='cuda:6', softmax_eval=True):
    y_pred = []
    model.eval()
    with torch.no_grad():
        for i, (ecg, target) in enumerate(loader):
            ecg = ecg.to(device=device) 
            # predict
            ecg_emb = model.encode_image(ecg)
...
def zeroshot_eval(model, set_name, device='cuda:6', args_zeroshot_eval=None):
